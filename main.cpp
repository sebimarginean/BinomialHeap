...
void showMenu() {
	cout << "Current heap is " << current_heap << endl
		 ...
		 << "\t8 Decrease the key in the current heap" << endl
		 << "\t9 Stop " << endl << endl
		 << "Choose (1-9): ";
}
...
int main() {
	...
	while (true) {
		...
		case 7:    // Extract the minimum element from current heap
			...
			break;
		case 8:    // Decrease the key to a new value
			if (current_heap > 0) {
				cout << "Type the old and new key separated by space(s): ";
				cin>>old_key>>new_key;
				decreaseKey(heaps[current_heap - 1]->head, old_key, new_key);
			}
			break;
		case 9:    // Stop
			n = heaps.size();
